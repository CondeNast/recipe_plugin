...
(function() {
	...
	tinymce.create('tinymce.plugins.amdEditZLRecipe', {
		init: function( editor, url ) {
			...
			editor.addButton( 'amdzlrecipe', {
				...
				onclick: function() {
					...
					var url = baseurl + '/wp-admin/media-upload.php?post_id=' + ((recipe_id) ? '1-' + recipe_id[1] : post_id) + '&type=amd_zlrecipe&tab=amd_zlrecipe&TB_iframe=true&width=640&height=523';
					alert(url);
		...
		execCommand: function(cmd, ui, type) {
			...
			var url = this.url;
			switch (cmd) {
				case "mceGetZLRecipeData":
					jQuery.post(ajaxurl, {action:'get_zlrecipe'}, function(data){
